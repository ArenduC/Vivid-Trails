  ...
  return (
    <div className="text-center p-8">
      ...
      <div
        ...
        className={`relative border-2 border-dashed rounded-2xl p-12 transition-all duration-300 ${isDragOver ? 'border-yellow-400 bg-slate-800/80' : 'border-slate-700 bg-slate-800/60'}`}
      >
        <input
          type="file"
          multiple
        ...
        <label htmlFor="file-upload" className="flex flex-col items-center justify-center space-y-4 cursor-pointer">
          ...
          <p className="text-xs text-slate-500">For best results, use original photos with location data enabled.</p>
        </label>
      </div>
    </div>
  );
