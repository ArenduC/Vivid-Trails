                ...
                <div className="px-8">
                    <div className="flex justify-between items-end -mt-16">
                        <div className="flex items-end gap-4">
                            <img src={profile.avatarUrl} alt={profile.username} className="w-32 h-32 rounded-full border-4 border-gray-900 bg-gray-800" />
                            <div>
                        ...
                        <div className="flex items-center gap-2 pb-4">
                            {isOwnProfile ? (
                                ...
                                <>
                                    <button className="bg-purple-600 hover:bg-purple-700 text-white font-bold py-2 px-4 rounded-full transition-colors">Follow</button>
                                    <button className="bg-gray-700 hover:bg-gray-600 text-white font-bold py-2 px-4 rounded-full transition-colors">Message</button>
                                </>
                            )}
                        </div>
                    </div>
                    <div className="mt-4 flex items-center gap-6 text-sm">
                        <p><span className="font-bold text-white">{profile.following.toLocaleString()}</span> <span className="text-gray-400">Following</span></p>
                        <p><span className="font-bold text-white">{(profile.followers / 1000).toFixed(1)}k</span> <span className="text-gray-400">Followers</span></p>
                    </div>
                </div>
            </div>
                ...
                <nav className="flex items-center gap-2">
                    {navItems.map(item => (
                        <button 
                            ...
                            className={`flex items-center gap-2 px-4 py-3 font-semibold transition-colors duration-200 border-b-2 ${activeTab === item.id ? 'text-purple-400 border-purple-400' : 'text-gray-400 border-transparent hover:text-white'}`}
                        >
                    ...
                    <div>
                        ...
                        {trips.length === 0 ? (
                            <div className="text-center py-20 px-8 bg-gray-800/50 rounded-2xl border border-dashed border-gray-700">
                                ...
                                <p className="text-gray-400 mb-6 max-w-md mx-auto">{isOwnProfile ? "You haven't created any trips. Why not share your first journey?" : `${profile.fullName} hasn't shared any trips.`}</p>
                                {isOwnProfile && (
                                     <button onClick={onNewTrip} className="flex items-center gap-2 bg-purple-600 hover:bg-purple-700 text-white font-bold py-2 px-6 rounded-full transition-transform transform hover:scale-105 shadow-lg mx-auto">
